    ...
    if(isset($_SESSION['ID'])){//renvoie les informations relatives à un utilisateur à partir de l'id stocké dans la variable de session
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if(isset($data->nom, $data->prenom, $data->genre, $data->mail, $data->tel, $data->societe, $data->numRue, $data->nomRue, $data->complement, $data->cp, $data->ville, $data->pays)){
                ...
                $user = new User($db);
                $id = $_SESSION['ID'];
                $regexMail = "/^((?!\.)[\w\-_.]*[^.])(@\w+)(\.\w+(\.\w+)?[^.\W])$/";

                if(preg_match($regexMail, $data->mail)){//si le mail respecte la regex, mais on peut rajouter d'autres contraines regex si besoin, c toi qui voit
                    try{
                        $user->updateInfos(
                        $id,
                        $data->nom,
                    ...
                    http_response_code(405);
                    echo json_encode(["response" => "Formulaire incorrect"]);
                }
